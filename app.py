...
app.layout = html.Div([
    ...
    html.H4("write what you want to trade"),
    dcc.Input(id='trade_type', value='AUDCAD', type='text'),
    # Numeric input for the trade amount
    html.H4("write how much you want to trade"),
    dcc.Input(id='trade-amt', value='20000', type='number'),
    # Numeric input for the limit price
    html.H4("write the limit price"),
    dcc.Input(id='limit_price', value='100', type='number'),
...
@app.callback(
    ...
    # We DON'T want to run this function whenever buy-or-sell, trade_type, or trade-amt is updated, so we pass those
    #   in as States, not Inputs:
    [State('mko-or-lmo','value'),State('buy-or-sell', 'value'), State('trade_type', 'value'), State('trade-amt', 'value')],
    # We DON'T want to start executing trades just because n_clicks was initialized to 0!!!
    prevent_initial_call=True
)
def trade(n_clicks, action, trade_type, trade_amt):  # Still don't use n_clicks, but we need the dependency
    ...
    msg = action + ' ' + trade_amt + ' ' + trade_type
